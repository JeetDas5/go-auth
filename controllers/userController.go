...
import (
	...
	"github.com/jeetdas5/golang-jwt/models"
...
func VerifyPassword(userPassword string, peovidedPassword string) (bool, string) {
	err := bcrypt.CompareHashAndPassword([]byte(peovidedPassword), []byte(userPassword))
	check := true
	if err != nil {
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"success": "User created successfully", "user_id": result.InsertedID})
...
func GetUsers() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := result.All(ctx, &allUsers); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "error occurred while reading user items",
			})
			return
		}
